  ...
  <div class="cart">
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" :checked="isAllChecked" @change="upCartChecked"/>
        <span>全选</span>
      </div>
      <div class="option">
...
export default {
  ...
  methods: {
    ...
    // 修改商品的勾选状态
    async upShopChecked(item) {
      item.isChecked = item.isChecked === 1 ? '0' : '1'
      try {
        await this.$store.dispatch('upChangeChecked', {
          skuId: item.skuId,
    ...
    async deleteCartById(item) {
      // console.log(item)
      try {
        await this.$store.dispatch('deleteShopCartList', item.skuId)
        this.getDate()
      } catch (error) {
        alert(error.message)
      }
    ...
    deleteAllCheckedCart() {
      this.cartInfoList.filter((item) => item.isChecked === 1).forEach(async(item) => {
        try {
          await this.$store.dispatch('deleteShopCartList', item.skuId)
          this.getDate()
        } catch (error) {
    ...
    upCartChecked(e) {
      this.cartInfoList.forEach(async(item) => {
        ...
        try {
          await this.$store.dispatch('upChangeChecked', { skuId: item.skuId, isChecked: item.isChecked })
        } catch (error) {
          alert(error.message)
        }
      })
  ...
  computed: {
    ...
    // 计算是否全选
    isAllChecked() {
      return this.cartInfoList.length > 0 ? this.cartInfoList.every((item) => item.isChecked === 1) : false
    },
    // 计算已选择商品
    selectedCount() {
