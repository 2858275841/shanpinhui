      ...
      <div class="checkout-tit">
        ...
        <div class="paymark">
          ...
          <span class="fr"
            ...
            ><em class="orange money">￥{{ OrderPayInfo.totalFee }}</em></span
          >
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getOrderPayInfo() {
      const result = await reqGetOrderPayInfo(this.$route.params.orderId)
      if (result.code === 200) {
    ...
    async open() {
      ...
      if (!this.timer) {
        this.timer = setInterval(async () => {
          const result = await reqGetPayStatus(this.$route.params.orderId)
          if (result.code === 200) {
            // 清除定时器
            clearInterval(this.timer)
            this.timer = null
